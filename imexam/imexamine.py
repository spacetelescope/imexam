...
class Imexamine(object):
    ...
    def __init__(self):
        ...
        #the second item in the tuple is the description of what the function does when that key is pressed
    ...
    def line_fit(self,x,y,form=None,subsample=4):
        ...
        ax.set_title("{0:s}  mean = {1:s}, fwhm= {2:s}".format(self.line_fit_pars["title"][0],str(fitmean),str(fwhm)))
    ...
    def column_fit(self,x,y,form=None,subsample=4):
        ...
        ax.set_title("{0:s}  mean = {1:s}, fwhm= {2:s}".format(self.column_fit_pars["title"][0],str(fitmean),str(fwhm)))
    ...
    def radial_profile_plot(self,x,y):
        ...
        for rad in range(1,rapert,1):
            aper_flux,annulus_sky,skysub_flux=self._aperture_phot(centerx,centery,radsize=rad,sky_inner=inner,skywidth=width,method="exact")
            radius.append(rad)
    ...
    def contour_plot(self,x,y):
        ...
        X,Y=np.meshgrid(np.arange(0,deltax,0.5)+x-deltax/2.,np.arange(0,deltay,0.5)+y-deltay/2.) #check this
    ...
    def surface_plot(self,x,y):
        ...
        if fancy:
            surf=ax.plot_surface(X,Y,Z,rstride=stride,cstride=stride,cmap=self.surface_pars["cmap"][0],alpha=0.6)             
        else:
            surf=ax.plot_surface(X,Y,Z,rstride=stride,cstride=stride,cmap=self.surface_pars["cmap"][0],linewidth=0,antialiased=False)
